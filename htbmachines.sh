...
function helpPanel(){
  ...
  echo -e "\t${purpleColour}s)${endColour}${grayColour} Buscar por las skills de la máquina${endColour}"
...
function getMachinesDifficulty(){
  ...
  if [ "$results_check" ]; then
    echo -e "\n${yellowColour}[+]${endColour}${grayColour} Representando las máquinas con la dificultad ${endColour}${blueColour}$difficulty${endColour}$grayColour:${endColour}\n"
...
function getOSMachines(){
  ...
  if [ "$os_results" ]; then
    ...
    echo -e "\n${redColour}[!] El sistema operativo $1 No existe${endColour}\n"
  fi
...
function getOSDifficultyMachines(){
  ...
  if [ "$check_results" ]; then
    echo -e "\n${yellowColour}[+]${endColour}${grayColour}Listando máquinas de dificultad${endColour}${blueColour} $difficulty${endColour}${grayColour} que tengan sistema operativo${endColour}${blueColour} $os${endColour}${yellowColour}:${endColour}\n"
...
function getSkill(){
  ...
  if [ "$check_skill" ]; then
    echo -e "\n${yellowColour}[+]${endColour}${grayColour} A continuación se representan las máquinas donde se emplea la Skill${endColour}${blueColour} $skill ${endColour}${yellowColour}:${endColour}\n"
